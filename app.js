...
function renderDetailDraft(){const box=$('#detailRows');box.innerHTML=detailDraft.map(detailEditor).join('');$('#detailCount').textContent=`${detailDraft.length} detail`;$$('.remove-detail').forEach(btn=>btn.onclick=()=>{if($$('.detail-card').length<=1)return toast('Minimal satu detail harus ada.');btn.closest('.detail-card').remove();detailDraft=collectDetails();renderDetailDraft();formDirty=true});$$('.pcs-button').forEach(btn=>btn.onclick=()=>{const p=btn.parentElement.querySelector('.pcs-picker');p.classList.toggle('hidden')});$$('.pcs-picker button').forEach(btn=>btn.onclick=()=>{const card=btn.closest('.detail-card'),hidden=card.querySelector('.d-pcs'),label=card.querySelector('.pcs-button');let n=btn.dataset.pcs==='more'?prompt('Masukkan PCS lebih dari 9:','10'):btn.dataset.pcs;if(n===null)return;n=Number(n);if(!Number.isInteger(n)||n<0||(btn.dataset.pcs==='more'&&n<10))return toast('PCS tidak valid.');hidden.value=n;label.textContent=`${n} pcs`;btn.parentElement.classList.add('hidden');formDirty=true});$$('.detail-card input').forEach(x=>x.oninput=()=>{formDirty=true})}
...
$('#addBtn').onclick=openAdd;$('#closeModal').onclick=()=>closeModal();$('#cancelModal').onclick=()=>closeModal();$('#addDetailBtn').onclick=()=>{detailDraft=collectDetails();detailDraft.push({id:uuid(),detail_order:detailDraft.length+1,damage:'',action:'',part:null,jumlah_pcs:0});renderDetailDraft();formDirty=true};$('#caseForm').onsubmit=saveCase;$('#deleteCaseBtn').onclick=startDelete;
